...
//<a href="https://blog.csdn.net/Kennethdroid/article/details/103931627">
...
void PBOSample::Init() {
    ...
    //PBO 绑定为 GL_PIXEL_UNPACK_BUFFER 时，glTexImage2D() 和 glTexSubImage2D() 表示从 PBO 中解包（unpack）像素数据并复制到帧缓冲区
    ...
    // PBO 绑定为 GL_PIXEL_PACK_BUFFER 时，glReadPixels() 表示从帧缓冲区中读取像素数据并打包进（pack） PBO 。
...
void PBOSample::UploadPixels() {
    ...
    BEGIN_TIME("PBOSample::UploadPixels Copy Pixels from System Mem to Textrure Obj")
        glBindTexture(GL_TEXTURE_2D, m_ImageTextureId);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_RenderImage.width, m_RenderImage.height, GL_RGBA, GL_UNSIGNED_BYTE,
                        m_RenderImage.ppPlane[0]);
...
void PBOSample::DownloadPixels() {
    ...
    BEGIN_TIME("DownloadPixels PBO glMapBufferRange")
        ...
        //通过 glMapBufferRange 获取 PBO 对应 GPU 缓冲区的内存地址。
