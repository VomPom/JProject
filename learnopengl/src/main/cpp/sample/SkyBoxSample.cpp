...
void SkyBoxSample::Draw(int screenW, int screenH) {
    ...
    if (!m_TextureId) {
        ...
        for (int i = 0; i < sizeof(m_pSkyBoxRenderImg) / sizeof(NativeImage); ++i) {
            ...
            //GL_TEXTURE_CUBE_MAP_POSITIVE_X代表了每面的位置
            //GL_TEXTURE_CUBE_MAP_POSITIVE_X	右
            //GL_TEXTURE_CUBE_MAP_NEGATIVE_X	左
            //GL_TEXTURE_CUBE_MAP_POSITIVE_Y	上
            //GL_TEXTURE_CUBE_MAP_NEGATIVE_Y	下GL_TEXTURE_CUBE_MAP
            //GL_TEXTURE_CUBE_MAP_POSITIVE_Z	后
            //GL_TEXTURE_CUBE_MAP_NEGATIVE_Z	前
            glTexImage2D(
                    GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
                    GL_RGBA, m_pSkyBoxRenderImg[i].width, m_pSkyBoxRenderImg[i].height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                    m_pSkyBoxRenderImg[i].ppPlane[0]
            );
        }
    ...
    //首先生成一个纹理，激活相应纹理单元，然后绑定到 GL_TEXTURE_CUBE_MAP类型纹理。
