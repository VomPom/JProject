...
void TextRenderSample::Draw(int screenW, int screenH) {
    ...
    RenderText(BYTE_FLOW, sizeof(BYTE_FLOW) / sizeof(BYTE_FLOW[0]) - 1, -0.9f, -0.2f, 1.0f, glm::vec3(0.7, 0.4f, 0.2f),
               viewport);
...
void TextRenderSample::RenderText(std::string text, GLfloat x, GLfloat y, GLfloat scale,
    ...
    for (c = text.begin(); c != text.end(); c++) {
        ...
        LOGCATE("TextRenderSample::RenderText [xpos,ypos,w,h]=[%f, %f, %f, %f], ch.advance >> 6 = %d", xpos, ypos, w, h,
                ch.advance >> 6);
...
void TextRenderSample::LoadFacesByUnicode(const wchar_t *text, int size) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        LOGCATE("TextRenderSample::LoadFacesByUnicode text[i]=%d [w,h,buffer]=[%d, %d, %p], advance.x=%ld", text[i],
                bitmap.width, bitmap.rows, bitmap.buffer, glyph->advance.x / MAX_SHORT_VALUE);
        // Set texture options
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
