...
void BlendingSample::Init() {
    ...
    char fShaderStr[] =
            ...
            "void main()\n"
            "{\n"
            "// FragColor = vec4(vec3(texture(s_TextureMap, v_texCoord)), 1.0);\n"
    ...
    windowsTrans.emplace_back(glm::vec3(-1.5f, 0.0f, -0.48f));
    windowsTrans.emplace_back(glm::vec3(1.5f, 0.0f, 0.51f));
    ...
    for (auto &windowsTran : windowsTrans) {
        GLfloat distance = std::sqrt(std::pow(windowsTran.x - 0.5f, 2.0f) + std::pow(windowsTran.y - 1.0f, 2.0f) +
...
void
BlendingSample::UpdateMatrix(glm::mat4 &mvpMatrix, int angleXRotate, int angleYRotate, float scale, glm::vec3 transVec3,
                             float ratio) {
    ...
    // Projection matrix
    //include.glm::mat4 Projection = include.glm::ortho(-ratio, ratio, -1.0f, 1.0f, 0.0f, 100.0f);
    //include.glm::mat4 Projection = include.glm::frustum(-ratio, ratio, -1.0f, 1.0f, 4.0f, 100.0f);
