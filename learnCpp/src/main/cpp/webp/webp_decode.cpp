 ...
 */
uint8_t *WebpDecoder::GetDecodedFrame() {
...
bool WebpDecoder::DecodeFrame() {
    ...
    int status = pthread_create(&worker, &attrib, ::DecodeFrame, this);
 ...
 */
bool WebpDecoder::DestroyDecoder() {
    if (state_ == state_decoding) {
        stopPending_ = true;
        return false;
    }
